...
"""
    ...
    8^n + 8^(n+1) ≡ 8^(n+2) (mod 11). 8 is called a Fibonacci primitive root
    of 11.

    Not every prime has a Fibonacci primitive root.
    
    There are 323 primes less than 10,000 with one or more Fibonacci primitive
    roots and the sum of these primes is 1,480,491.
...
def problem():
    ...
    for p in primes:
        for n in xrange(0, 10):
            fib_root = True
            seq = [n**x % p for x in xrange(0, 10)]
            for i in xrange(len(seq)):
                if (seq[i] + seq[(i + 1) % 10]) % p != seq[(i + 2) % 10]:
                    fib_root = False
                    break
            if fib_root:
                total += p
