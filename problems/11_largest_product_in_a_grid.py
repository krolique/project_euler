...
# max length of the sub vector
n = 4
# multiply list items, assuming they are all ints
multiply = lambda x: reduce(lambda y, z: y*z, x)


def find_max(i=0, j=0):
    """ """

    try:
        down = multiply(A[i+x][j] for x in range(n))
    except IndexError:
        down = 0

    try:
        left = multiply(A[i][j-x] for x in range(n))
    except IndexError:
        left = 0

    try:
        right = multiply(A[i][j+x] for x in range(n))
    except IndexError:
        right = 0

    try:
        up = multiply(A[i-x][j] for x in range(n))
    except IndexError:
        up = 0

    try:
        down_right_diag = multiply(A[i-x][j+x] for x in range(n))
    except IndexError:
        down_right_diag = 0

    try:
        down_left_diag = multiply(A[i-x][j-x] for x in range(n))
    except IndexError:
        down_right_diag = 0


    try:
        up_right_diag = multiply(A[i+x][j+x] for x in range(n))
    except IndexError:
        up_right_diag = 0

    try:
        up_left_diag = multiply(A[i+x][j-x] for x in range(n))
    except IndexError:
        up_left_diag = 0


    return max(down, left, right, up, down_right_diag, down_left_diag,
               up_right_diag, up_left_diag)
